...
document.addEventListener('DOMContentLoaded', () => {
  ...
  el('formLogin').addEventListener('submit', (e) => {
    e.preventDefault();
    const r = Auth.login(el('loginUsuario').value, el('loginSenha').value);
  ...
  el('formCadastro').addEventListener('submit', (e) => {
    e.preventDefault();
    const r = Auth.registrar(el('cadUsuario').value, el('cadSenha').value);
  ...
  /* ---------- Vincular personagem ---------------------------------------- */
  el('btnVincular').addEventListener('click', () => {
    const id = el('selectVincular').value;
    if (!id) return;
    const r = Auth.vincular(id);
  ...
  el('formNovoPersonagem').addEventListener('submit', (e) => {
    e.preventDefault();
    const r = Roster.cadastrarNovo(el('novoNome').value, el('novaImg').value);
  ...
  function renderCharGrid() {
    ...
    meus.forEach(p => {
      ...
      card.append(avatarBtn, nome, tag, btnEditar);
  ...
  el('btnSalvarFicha').addEventListener('click', () => {
    if (!editingId) return;
    const atributos = {};
    ...
    const r = Auth.salvarFicha(editingId, atributos);
